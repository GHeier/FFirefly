...
int binary_search(float x_val, vector<float> &x) {
    // Performs a binary search to find the index of the largest element in x that is less than x_val
    // x_val: value to search for
    // x: vector of values to search in
    // returns: index of the largest element in x that is less than x_val
...
complex<Vec> interpolate_3D(float x_val, float y_val, float z_val, float x_min, float x_max, float y_min, float y_max, float z_min, float z_max, int nx, int ny, int nz, vector<complex<Vec>> &f) {
    ...
    complex<Vec> result = (1 - x_rel) * (1 - y_rel) * (1 - z_rel) * f[i*nx*nx+j*ny+k] +
                   x_rel * (1 - y_rel) * (1 - z_rel) * f[(i + 1)*nx*nx+j*ny+k] +
                   (1 - x_rel) * y_rel * (1 - z_rel) * f[i*nx*nx + (j + 1)*ny+k] +
                   x_rel * y_rel * (1 - z_rel) * f[(i + 1)*nx*nx+(j + 1)*ny+k] +
                   (1 - x_rel) * (1 - y_rel) * z_rel * f[i*nx*nx+j*ny+k + 1] +
