...
mutable struct ComplexScalarField
    cmf::Ptr{Cvoid}
    function ComplexScalarField(cmf::Ptr{Cvoid})
        new(cmf)
    end

    function ComplexScalarField(filename::String)
    ...
    function destroy!(csf::ComplexScalarField)
        ccall((:destroy_CMF_CS, testmod), Cvoid, (Ptr{Cvoid},), csf.cmf)
    end
end

function Base.finalize(csf::ComplexScalarField)
    destroy!(csf)
end
...
function (self::ComplexScalarField)(k::Vector{Float64}, w::Float64)
...
function (self::ComplexScalarField)(w::Float64)
    real_result::Ref{Float64} = Ref(Float64(0.0))
    imag_result::Ref{Float64} = Ref(Float64(0.0))
...
cmf2 = Field.ComplexScalarField(file)
if cmf2 == C_NULL
...
cmf3 = Field.ComplexScalarField("/home/g/Research/Materials/Test/DOS.dat")
if cmf3 == C_NULL
