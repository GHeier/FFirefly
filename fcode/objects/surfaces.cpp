...
vector<Vec> points_from_indices(function<float(Vec)> func, int i, int j, int k, vector<int> k_mesh) {
    Vec p1 = brillouin_zone * Vec(1.0*i/k_mesh[0] - 0.5, 1.0*j/k_mesh[1] - 0.5, 1.0*k/k_mesh[2] - 0.5);
    Vec p2 = brillouin_zone * Vec(1.0*(i+1)/k_mesh[0] - 0.5, 1.0*j/k_mesh[1] - 0.5, 1.0*k/k_mesh[2] - 0.5);
    Vec p3 = brillouin_zone * Vec(1.0*(i+1)/k_mesh[0] - 0.5, 1.0*(j+1)/k_mesh[1] - 0.5, 1.0*k/k_mesh[2] - 0.5);
    Vec p4 = brillouin_zone * Vec(1.0*i/k_mesh[0] - 0.5, 1.0*(j+1)/k_mesh[1] - 0.5, 1.0*k/k_mesh[2] - 0.5);
    Vec p5 = brillouin_zone * Vec(1.0*i/k_mesh[0] - 0.5, 1.0*j/k_mesh[1] - 0.5, 1.0*(k+1)/k_mesh[2] - 0.5);
    Vec p6 = brillouin_zone * Vec(1.0*(i+1)/k_mesh[0] - 0.5, 1.0*j/k_mesh[1] - 0.5, 1.0*(k+1)/k_mesh[2] - 0.5);
    Vec p7 = brillouin_zone * Vec(1.0*(i+1)/k_mesh[0] - 0.5, 1.0*(j+1)/k_mesh[1] - 0.5, 1.0*(k+1)/k_mesh[2] - 0.5);
    Vec p8 = brillouin_zone * Vec(1.0*i/k_mesh[0] - 0.5, 1.0*(j+1)/k_mesh[1] - 0.5, 1.0*(k+1)/k_mesh[2] - 0.5);
...
vector<Vec> points_from_indices_2d(function<float(Vec)> func, int i, int j, int k, vector<int> k_mesh) {
    Vec p1 = brillouin_zone * Vec(1.0*i/k_mesh[0] - 0.5, 1.0*j/k_mesh[1] - 0.5);
    Vec p2 = brillouin_zone * Vec(1.0*(i+1)/k_mesh[0] - 0.5, 1.0*j/k_mesh[1] - 0.5);
    Vec p3 = brillouin_zone * Vec(1.0*(i+1)/k_mesh[0] - 0.5, 1.0*(j+1)/k_mesh[1] - 0.5);
    Vec p4 = brillouin_zone * Vec(1.0*i/k_mesh[0] - 0.5, 1.0*(j+1)/k_mesh[1] - 0.5);
...
vector<Vec> get_endpoints(vector<Vec> points, function<float(Vec k)> func, float s_val) {
    // y = m * x + b to find the points on the surface
    Vec k1 = points[0]; Vec k2 = points[1]; 
    Vec k3 = points[2]; Vec k4 = points[3];
    float ep1 = func(k1); float ep2 = func(k2); 
    float ep3 = func(k3); float ep4 = func(k4);
    ...
    else if ( is_between(ep1, s_val, ep2) and is_between(ep4, s_val, ep3) ) { // Case 2
        return_points[0] = (k2-k1) * (s_val - ep1) / (ep2 - ep1) + k1;
        return_points[1] = (k4-k3) * (s_val - ep3) / (ep4 - ep3) + k3;
    }
    else if ( is_between(ep2, s_val, ep1) and is_between(ep2, s_val, ep3) ) { // Case 3
        return_points[0] = (k2-k1) * (s_val - ep1) / (ep2 - ep1) + k1;
        return_points[1] = (k3-k2) * (s_val - ep2) / (ep3 - ep2) + k2;
    }
    else if ( is_between(ep1, s_val, ep4) and is_between(ep2, s_val, ep3) ) { // Case 4
        return_points[0] = (k4-k1) * (s_val - ep1) / (ep4 - ep1) + k1;
        return_points[1] = (k3-k2) * (s_val - ep2) / (ep3 - ep2) + k2;
    }
    else if ( is_between(ep2, s_val, ep3) and is_between(ep4, s_val, ep3) ) { // Case 5
        return_points[0] = (k3-k2) * (s_val - ep2) / (ep3 - ep2) + k2;
        return_points[1] = (k4-k3) * (s_val - ep3) / (ep4 - ep3) + k3;
    }
    else if ( is_between(ep1, s_val, ep4) and is_between(ep4, s_val, ep3) ) { // Case 6
        return_points[0] = (k4-k1) * (s_val - ep1) / (ep4 - ep1) + k1;
