...
int binary_search(float x_val, vector<float> &x) {
    // Performs a binary search to find the index of the largest element in x
    // that is less than x_val x_val: value to search for x: vector of values to
    // search in returns: index of the largest element in x that is less than
    // x_val
...
float interpolate_2D(float x_val, float y_val, float x_min, float x_max,
                     float y_min, float y_max, vector<vector<float>> &f) {
    ...
    // interpolated value of f(x_val, y_val)
    ...
    if (f.size() < 2)
        throw invalid_argument("f size too small");
    if (f[0].size() < 2)
        throw invalid_argument("f[0] size too small");
...
float interpolate_3D(float x_val, float y_val, float z_val, float x_min,
                     float x_max, float y_min, float y_max, float z_min,
                     float z_max, vector<vector<vector<float>>> &f) {
    // Interpolates a 3D function f(x, y, z) given on a grid between x_min and
    // x_max and y_min and y_max and z_min and z_max at the point (x_val, y_val,
    // z_val) using trilinear interpolation x_val, y_val, z_val: values at which
    // to interpolate x_min, x_max: minimum and maximum values of x y_min,
    // y_max: minimum and maximum values of y z_min, z_max: minimum and maximum
    // values of z f: vector of function values at the grid points returns:
    // interpolated value of f(x_val, y_val, z_val)
    ...
    if (f[0].size() < 2)
        throw invalid_argument("f[0] size too small");
    if (f[0][0].size() < 2)
        throw invalid_argument("f[0][0] size too small");
...
float interpolate_4D(float x_val, float y_val, float z_val, float w_val,
                     ...
                     float z_min, float z_max, float w_min, float w_max,
    ...
    if (f[0][0].size() < 2)
        throw invalid_argument("f[0][0] size too small");
    if (f[0][0][0].size() < 2)
        throw invalid_argument("f[0][0][0] size too small");
    ...
    if (k < 0 || k >= f[0][0].size())
        throw out_of_range("k out of bounds");
    if (l < 0 || l >= f[0][0][0].size())
        throw out_of_range("l out of bounds");
...
complex<float> interpolate_1D_complex(float x_val, float x_min, float x_max,
                                      vector<complex<float>> &f) {
    // Interpolates a 1D complex function f(x) given on a grid between x_min and
    // x_max at the point x_val using linear interpolation x_val: value at which
    // to interpolate x_min, x_max: minimum and maximum values of x f: vector of
    // function values at the grid points returns: interpolated value of
    // f(x_val)
...
complex<float> interpolate_2D_complex(float x_val, float y_val, float x_min,
                                      float x_max, float y_min, float y_max,
                                      vector<vector<complex<float>>> &f) {
    // Interpolates a 2D complex function f(x, y) given on a grid between x_min
    // and x_max and y_min and y_max at the point (x_val, y_val) using bilinear
    // interpolation x_val, y_val: values at which to interpolate x_min, x_max:
    // minimum and maximum values of x y_min, y_max: minimum and maximum values
    // of y f: vector of function values at the grid points returns:
    // interpolated value of f(x_val, y_val)
...
complex<float>
interpolate_3D_complex(float x_val, float y_val, float z_val, float x_min,
                       float x_max, float y_min, float y_max, float z_min,
                       float z_max, vector<vector<vector<complex<float>>>> &f) {
    // Interpolates a 3D complex function f(x, y, z) given on a grid between
    // x_min and x_max and y_min and y_max and z_min and z_max at the point
    // (x_val, y_val, z_val) using trilinear interpolation x_val, y_val, z_val:
    // values at which to interpolate x_min, x_max: minimum and maximum values
    // of x y_min, y_max: minimum and maximum values of y z_min, z_max: minimum
    // and maximum values of z f: vector of function values at the grid points
    ...
    complex<float> fx2 =
        (1 - x_rel) * f[i][j + 1][k] + x_rel * f[i + 1][j + 1][k];
    complex<float> fx3 =
        (1 - x_rel) * f[i][j][k + 1] + x_rel * f[i + 1][j][k + 1];
    complex<float> fx4 =
        (1 - x_rel) * f[i][j + 1][k + 1] + x_rel * f[i + 1][j + 1][k + 1];
...
complex<float>
interpolate_4D_complex(float x_val, float y_val, float z_val, float w_val,
                       float x_min, float x_max, float y_min, float y_max,
                       float z_min, float z_max, float w_min, float w_max,
                       vector<vector<vector<vector<complex<float>>>>> &f) {
    // Interpolates a 4D complex function f(x, y, z, w) given on a grid between
    // x_min and x_max and y_min and y_max and z_min and z_max and w_min and
    // w_max at the point (x_val, y_val, z_val, w_val) using trilinear
    // interpolation x_val, y_val, z_val, w_val: values at which to interpolate
    // x_min, x_max: minimum and maximum values of x
    // y_min, y_max: minimum and maximum values of y
    // z_min, z_max: minimum and maximum values of z
    // w_min, w_max: minimum and maximum values of w
    ...
    complex<float> fx1 =
        f[i][j][k][l] * (1 - x_rel) + f[i + 1][j][k][l] * x_rel;
    complex<float> fx2 =
        f[i][j + 1][k][l] * (1 - x_rel) + f[i + 1][j + 1][k][l] * x_rel;
    complex<float> fx3 =
        f[i][j][k + 1][l] * (1 - x_rel) + f[i + 1][j][k + 1][l] * x_rel;
    complex<float> fx4 =
        f[i][j + 1][k + 1][l] * (1 - x_rel) + f[i + 1][j + 1][k + 1][l] * x_rel;
    complex<float> fx5 =
        f[i][j][k][l + 1] * (1 - x_rel) + f[i + 1][j][k][l + 1] * x_rel;
    complex<float> fx6 =
        f[i][j + 1][k][l + 1] * (1 - x_rel) + f[i + 1][j + 1][k][l + 1] * x_rel;
    complex<float> fx7 =
        f[i][j][k + 1][l + 1] * (1 - x_rel) + f[i + 1][j][k + 1][l + 1] * x_rel;
    complex<float> fx8 = f[i][j + 1][k + 1][l + 1] * (1 - x_rel) +
                         f[i + 1][j + 1][k + 1][l + 1] * x_rel;
...
float interpolate_2D(float x_val, float y_val, float x_min, float x_max,
                     float y_min, float y_max, int nx, int ny,
                     vector<float> &f) {
    // Interpolates a 2D function f(x, y) given on a grid between x_min and
    // x_max and y_min and y_max at the point (x_val, y_val) using bilinear
    // interpolation x_val, y_val: values at which to interpolate x_min, x_max:
    ...
    if (y_val < y_min || y_val > y_max)
        throw out_of_range("y_val out of bounds");
    if (f.size() != nx * ny)
        throw invalid_argument("f size does not match nx and ny");
...
float interpolate_3D(float x_val, float y_val, float z_val, float x_min,
                     float x_max, float y_min, float y_max, float z_min,
                     float z_max, int nx, int ny, int nz, vector<float> &f) {
    // Interpolates a 3D function f(x, y, z) given on a grid between x_min and
    // x_max and y_min and y_max and z_min and z_max at the point (x_val, y_val,
    // z_val) using trilinear interpolation x_val, y_val, z_val: values at which
    // to interpolate x_min, x_max: minimum and maximum values of x y_min,
    ...
    float result =
        (1 - x_rel) * (1 - y_rel) * (1 - z_rel) * f[i * nx * nx + j * ny + k] +
...
float interpolate_4D(float x_val, float y_val, float z_val, float w_val,
                     float x_min, float x_max, float y_min, float y_max,
                     float z_min, float z_max, float w_min, float w_max, int nx,
                     int ny, int nz, int nw, vector<float> &f) {
    // Interpolates a 4D function f(x, y, z, w) given on a grid between x_min
    // and x_max and y_min and y_max and z_min and z_max and w_min and w_max at
    // the point (x_val, y_val, z_val, w_val) using trilinear interpolation
    // x_val, y_val, z_val, w_val: values at which to interpolate
...
complex<Vec> interpolate_1D(float x_val, float x_min, float x_max,
    ...
    if (x_rel < 0 || x_rel > 1)
        throw out_of_range("x_rel out of bounds");
    complex<Vec> result = f[i] + x_rel * (f[i + 1] - f[i]);

    return result;
}

complex<Vec> interpolate_2D(float x_val, float y_val, float x_min, float x_max,
                            float y_min, float y_max, int nx, int ny,
                            vector<complex<Vec>> &f) {
    // Interpolates a 2D function f(x, y) given on a grid between x_min and
    // x_max and y_min and y_max at the point (x_val, y_val) using bilinear
...
complex<Vec> interpolate_3D(float x_val, float y_val, float z_val, float x_min,
                            float x_max, float y_min, float y_max, float z_min,
                            float z_max, int nx, int ny, int nz,
                            vector<complex<Vec>> &f) {
    // Interpolates a 3D function f(x, y, z) given on a grid between x_min and
    // x_max and y_min and y_max and z_min and z_max at the point (x_val, y_val,
    // z_val) using trilinear interpolation x_val, y_val, z_val: values at which
