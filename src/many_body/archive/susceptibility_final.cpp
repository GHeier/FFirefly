...
public:
    ...
    double fermi_energy() const { return 0.0; } // Fermi level at 0 with mu in dispersion
};
...
int main() {
    ...
    std::cout << "2D Tight-Binding Model: E(k) = -2t[cos(kx) + cos(ky)] - mu\n";
    ...
    std::ofstream outfile("chi_final.dat");
    outfile << "# q/(pi,pi)  chi_tetrahedron  chi_numerical  ratio\n";

    for (int iq = 0; iq <= 20; iq++) {
        double q_mag = (iq * PI / tb.a) / 20.0;
    ...
    outfile.close();
    std::cout << "\nResults saved to chi_final.dat\n";
