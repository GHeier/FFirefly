...
void make_values_2d(vector<vector<complex<Vec>>> &values, CMData &data) {
  ...
  for (int i = 0; i < data.n_inds.size(); i++) {
    ...
    if (end == 0)
      break;
    values.emplace_back(data.values.begin() + start, data.values.begin() + end);
  }
}
...
complex<Vec> CMField::operator()(int n, Vec point, float w) {
  ...
  if (!data.with_w) {
    ...
    if (data.dimension == 2)
      return CMF_search_3d(p.x, p.y, w, nx, ny, data.w_points, values[n - 1]);
    if (data.dimension == 3)
      return CMF_search_4d(p.x, p.y, p.z, w, nx, ny, nz, data.w_points,
                           values[n - 1]);
  }
...
complex<Vec> CMF_search_2d(float x_val, float w_val, int nx,
  ...
  // and y_min and y_max at the point (x_val, y_val) using bilinear
  // interpolation x_val, y_val: values at which to interpolate x_min, x_max:
  // minimum and maximum values of x y_min, y_max: minimum and maximum values of
  // y f: vector of function values at the grid points returns: interpolated
  // value of f(x_val)
...
complex<Vec> CMF_search_3d(float x_val, float y_val, float w_val, int nx,
  ...
  // to interpolate x_min, x_max: minimum and maximum values of x y_min, y_max:
  // minimum and maximum values of y w_min, w_max: minimum and maximum values of
  // w f: vector of function values at the grid points returns: interpolated
  // value of f(x_val, y_val, w_val)
  float x_min = 0, x_max = 1;
...
complex<Vec> CMF_search_4d(float x_val, float y_val, float z_val, float w_val,
                           ...
                           vector<complex<Vec>> &f) {
  // Interpolates a 4D function f(x, y, z, w) given on a grid between x_min and
  // x_max and y_min and y_max and z_min and z_max and w_min and w_max at the
  // point (x_val, y_val, z_val, w_val) using trilinear interpolation x_val,
  // y_val, z_val, w_val: values at which to interpolate x_min, x_max: minimum
  // and maximum values of x y_min, y_max: minimum and maximum values of y
