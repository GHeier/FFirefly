...
void make_values_2d(vector<vector<complex<Vec>>> &values, CMData &data) {
    ...
    for (int i = 0; i < data.n_inds.size() - 1; i++) {
...
void CMF::find_domain(CMData &data) {
    ...
    int first_n = -1;
    if (data.n_inds.size() > 0) first_n = data.n_inds[1];
    ...
    for (int i = jump; i < data.points.size(); i += jump) {
        if (data.with_n and i >= first_n) break;
...
complex<Vec> CMF::operator() (float w) {
    ...
    if (data.with_n) throw runtime_error("This field requires an index to be specified.");
    return CMF_search_1d(w, data.w_points, values[0]);
}

complex<Vec> CMF::operator() (int n, float w) {
    if (!data.with_w) throw runtime_error("This field does not have a w dimension.");
    if (!data.with_n) throw runtime_error("This field forbids an index to be specified.");
    if (n < 1 || n > values.size()) throw out_of_range("Index starts at 1 and is out of bounds.");
...
complex<Vec> CMF::operator() (Vec point, float w) {
    ...
    if (!data.with_w) {
        ...
        if (data.dimension == 3) 
            return interpolate_3D(p.x, p.y, p.z, 0, 1, 0, 1, 0, 1, nx, ny, nz, values[0]);
    }
    else {
        if (data.dimension == 1) 
        ...
        if (data.dimension == 3) 
            return CMF_search_4d(p.x, p.y, p.z, w, nx, ny, nz, data.w_points, values[0]);
    }
...
complex<Vec> CMF::operator() (int n, Vec point, float w) {
    ...
    if (!data.with_w) {
        ...
        if (data.dimension == 3) 
            return interpolate_3D(p.x, p.y, p.z, 0, 1, 0, 1, 0, 1, nx, ny, nz, values[n-1]);
    }
    else {
        if (data.dimension == 1) 
        ...
        if (data.dimension == 3) 
            return CMF_search_4d(p.x, p.y, p.z, w, nx, ny, nz, data.w_points, values[n-1]);
    }
    throw runtime_error("Invalid dimension.");
}
