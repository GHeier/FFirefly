...
vector<Vec> points_from_indices(function<float(Vec)> func, int i, int j, int k,
                                vector<int> k_mesh) {
    Vec p1 = brillouin_zone * Vec(1.0 * i / k_mesh[0] - 0.5,
                                  1.0 * j / k_mesh[1] - 0.5,
                                  1.0 * k / k_mesh[2] - 0.5);
    ...
    Vec p4 = brillouin_zone * Vec(1.0 * i / k_mesh[0] - 0.5,
                                  1.0 * (j + 1) / k_mesh[1] - 0.5,
                                  1.0 * k / k_mesh[2] - 0.5);
    Vec p5 = brillouin_zone * Vec(1.0 * i / k_mesh[0] - 0.5,
                                  1.0 * j / k_mesh[1] - 0.5,
    ...
    for (int i = 0; i < k_mesh[0]; i++) {
        for (int j = 0; j < k_mesh[1]; j++) {
            for (int k = 0; k < k_mesh[2]; k++) {
                ...
                // Finds every k-space point in all possible tetrahedra, along
                // with its associated area in the surface, using the above
                // functions
                for (int c = 0; c < 6; c++) {
                    ...
                    // cout << "Corner Points: " << corner_points.size() <<
                    // endl; cout << "Corner Points: " << corner_points[0] <<
                    // endl; cout << "Corner Points: " << corner_points[1] <<
                    // endl; cout << "Corner Points: " << corner_points[2] <<
                    // endl;

                    // Averages the corner points to find the center of the
                    // triangle
                    Vec average;
                    float b = 0;
...
pair<int, int> get_index_and_length(float L, float U,
    ...
    int lower_index =
        std::lower_bound(sortedList.begin(), sortedList.end(), L) -
        sortedList.begin();
    ...
    for (int i = lower_index; i < sortedList.size() and sortedList[i] <= U;
         i++) {
